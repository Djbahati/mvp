...
import { Eye, EyeOff, Lock, Mail, User, Phone, Globe, ArrowRight, ShieldCheck, X } from 'lucide-react';

export const SignupView: React.FC = () => {
  const { signup, socialLogin, setAuthView } = useAuth();
  ...
  const handleSocialSubmit = async (e: React.FormEvent) => {
    ...
    setError('');
    if (!socialEmail) {
      setError('Please enter your email address for social sign up.');
      return;
    }
    setLoading(true);
    try {
  ...
  return (
    <div className="w-full max-w-lg mx-auto bg-slate-900 border border-slate-800 rounded-3xl p-8 shadow-2xl relative">
      ...
      <div className="mb-6">
        ...
        <div className="relative flex py-1 items-center">
          ...
          <span className="flex-shrink mx-4 text-slate-500 text-[11px] uppercase tracking-wider">Or register with email</span>
          <div className="flex-grow border-t border-slate-800"></div>
        </div>
      </div>
        ...
        <div className="absolute inset-0 bg-slate-950/90 backdrop-blur-sm z-50 rounded-3xl p-6 flex flex-col justify-center">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-base font-bold text-white capitalize">
              Sign up with {socialModalProvider}
            </h3>
            <button
          ...
          <form onSubmit={handleSocialSubmit} className="space-y-4">
            ...
            >
              {loading ? (
                ...
                <span>Complete Social Sign Up</span>
              )}
            </button>
          </form>
        </div>
